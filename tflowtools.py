...
# copypasta from tut1
def quickrun(operators, grabbed_vars = None, dir='probeview',
              session=None, feed_dict=None,step=1,show_interval=1):
    if type(operators) != list:
        operators = [operators]

    sess = session if session else gen_initialized_session(dir=dir)

    results = sess.run([operators,grabbed_vars],feed_dict=feed_dict)
    if (show_interval and step % show_interval) == 0:
        show_results(results[1], grabbed_vars, dir)
